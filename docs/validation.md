...
## 2. Raw data smoothing and sampling
## 2. 原始数据平滑与采样
...
- **预期结果**：终端应打印 `Saved trajectory visualization to .../raw_scene.png`，且 PNG 文件被创建。
...
- **排查建议**：若图片为空，请确认 `--recording-id` 与 CSV 匹配，并尝试添加 `--tracks 1 2 3` 以减少杂乱。
...
- **Expected**: Split Parquet files (for example, `train.parquet`) and matching `*_windows.pkl` appear.
- **预期结果**：应生成拆分后的 Parquet 文件（如 `train.parquet`）以及对应的 `*_windows.pkl`。
...
- **排查建议**：若未生成窗口，请确认配置中的原始路径存在且符合 highD 目录结构。

## 5. Processed data self-check
## 5. 预处理自检
...
- **预期结果**：输出示例：
  ...
  ```
- **Troubleshooting**: Large Frenet errors hint at bad centerlines; DRAC outliers suggest extreme braking or missing leader speeds; neighbor mismatches mean the center-frame snapshot may be incomplete—re-run preprocessing with correct `neighbor_radius_s`.
- **排查建议**：Frenet 误差大通常意味着中心线质量差；DRAC 异常可能由极端制动或缺失前车速度导致；邻居不匹配说明中心帧快照不完整，可使用正确的 `neighbor_radius_s` 重新预处理。
